...
try {
    ...
    // 检查群聊是否被免打扰
    $is_muted = false;
    $stmt = $conn->prepare("SELECT is_muted FROM chat_settings WHERE user_id = ? AND chat_type = 'group' AND chat_id = ?");
    ...
    echo json_encode(['success' => true, 'messages' => $messages, 'is_muted' => $is_muted]);
} catch (Exception $e) {
    // 捕获所有异常并返回错误信息
